...
// 検索した本がすでDBに存在している場合、APIを叩かずにDBにアクセス
func TestSearchBooksAlreadyRecordsExistNoApiOK(t *testing.T) {
	gin.SetMode(gin.TestMode)
	db, _ := infra.NewTestDB()
	defer func() {
		db, _ := db.DB()
	...
	req, _ := http.NewRequest("GET", "/book/search?title=test&maxNum=1", nil)
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
